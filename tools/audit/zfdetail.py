#!/usr/bin/env python3
"""Which meshes in a weapon are z-fighting, and where.

fullcheck.py counts z-fights per weapon; it cannot tell you which two boxes are at fault, which
...
call that built it.
    ...
    python3 tools/audit/zfdetail.py Seax Khopesh     # just these (substring, case-insensitive)
...
separate them and they flicker at every distance. A pair with a small non-zero gap may never be
visible -- fix those second, and only where the model is drawn large.
"""
import sys, json, pathlib

sys.path.insert(0, str(pathlib.Path(__file__).resolve().parent))
from audit import build_ctx, COPLANAR_EPS  # noqa: E402
